...
#define FOO_RETURNS_NONNULL __attribute__((returns_nonnull))
...
#ifndef FOO_BIT
#define FOO_BIT(x, n) (((x) >> (n)) & 1)
#endif

#ifndef FOO_BIT_SET
#define FOO_BIT_SET(x, n)       \
    ({                          \
        __typeof__(x) _x = (1); \
        (x) |= (_x << (n));     \
    })
...
#ifndef FOO_BIT_CLEAR
#define FOO_BIT_CLEAR(x, n)     \
    ({                          \
        __typeof__(x) _x = (1); \
        (x) &= ~(_x << (n));    \
    })
...
#define FOO_SW_MEMBARRIER() asm volatile("" : : : "memory")
